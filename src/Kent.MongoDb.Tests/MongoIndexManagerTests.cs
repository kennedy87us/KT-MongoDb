namespace Kent.MongoDb.Tests
{
    ...
    public class MongoIndexManagerTests
    {
        ...
        public void CreateIndexesIfMissing_NotExists_Then_CreateNewIndex()
        {
            ...
            var indexManager = new MongoIndexManager(null, _mockContext.Object);
            ...
            _mockAsyncCursor.Setup(m => m.Current).Returns(new[]
            {
                new BsonDocument(
                    new List<BsonElement> {
                        new BsonElement("v", 2),
                        new BsonElement("key", new BsonDocument(
                            new List<BsonElement> { new BsonElement(nameof(Test.Id), 1), new BsonElement(nameof(Test.Value), 1) })
                        ),
                        new BsonElement("name", indexName)
                    }
                )
            });
            var indexManager = new MongoIndexManager(null, _mockContext.Object);
        ...
        public void GetIndexExistingState_Returns_NotExists()
        {
            ...
            {
                new MongoIndexField(nameof(Test.Value)), new MongoIndexField(nameof(Test.RefId))
            });
        ...
        public void GetIndexExistingState_Returns_AlreadyExists()
        {
            ...
            _mockAsyncCursor.Setup(m => m.Current).Returns(new[]
            {
                new BsonDocument(
                    new List<BsonElement> {
                        new BsonElement("v", 2),
                        new BsonElement("key", new BsonDocument(
                            new List<BsonElement> { new BsonElement(nameof(Test.Id), 1), new BsonElement(nameof(Test.RefId), 1) })
                        ),
                        new BsonElement("name", indexName)
                    }
                )
            ...
            var index = new MongoIndex(indexName, new List<MongoIndexField>
            {
                new MongoIndexField(nameof(Test.Id)), new MongoIndexField(nameof(Test.RefId))
            });
            var actual = indexManager.GetIndexExistingState<Test>(index);
        ...
        public void GetIndexExistingState_Returns_ExistsWithDifferentName()
        {
            ...
            {
                new MongoIndexField(nameof(Test.Id)), new MongoIndexField(nameof(Test.Value))
            });
            var actual = indexManager.GetIndexExistingState<Test>(index);
        ...
        public void DropAllIndexes_NoException()
        {
            ...
            // Assert
            _mockIndexManager.Verify(m => m.DropAll(It.IsAny<DropIndexOptions>(), It.IsAny<CancellationToken>()), Times.Once());
        }
        ...
        public async void DropAllIndexesAsync_NoException()
        {
            ...
            // Assert
            _mockIndexManager.Verify(m => m.DropAllAsync(It.IsAny<DropIndexOptions>(), It.IsAny<CancellationToken>()), Times.Once());
        }
        ...
        public void DropIndex_NoException()
        {
            ...
            // Assert
            _mockIndexManager.Verify(m => m.DropOne(indexName, It.IsAny<DropIndexOptions>(), It.IsAny<CancellationToken>()), Times.Once());
        }
        ...
        public void DropIndexAsync_NoException()
        {
            ...
            // Assert
            _mockIndexManager.Verify(m => m.DropOneAsync(indexName, It.IsAny<DropIndexOptions>(), It.IsAny<CancellationToken>()), Times.Once());
        }
    }
}